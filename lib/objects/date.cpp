...
Date::Date(string date){
	if(date.find(' ') != string::npos){
		...
		this->extractDate(dt[0]);
		this->extractTime(dt[1]);
		delete dt;
...
void Date::extractDate(string date){
	...
	this->year = (int16_t)atoi(dat[0].c_str());
	this->month = (int8_t)atoi(dat[1].c_str());
	this->day = (int8_t)atoi(dat[2].c_str());
	delete dat;
}

void Date::extractTime(string time){
	vector<string>* dat = c_explode(':',time);
	this->hour = (int8_t)atoi(dat[0].c_str());
	this->min = (int8_t)atoi(dat[1].c_str());
	this->sec = (int8_t)atoi(dat[2].c_str());
	delete dat;
}
